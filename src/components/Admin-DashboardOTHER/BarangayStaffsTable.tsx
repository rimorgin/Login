...
import StaffDataService from '../../services/staff.services';
  ...
  export default function BrgyStaffsTable({ data, onDataChange }: UsersTableProps) {
    const theme = useMantineTheme();
    const isLargeScreen = window.innerWidth >= 1024;
    const [isEditing, setIsEditing] = useState(false); 
    const [isDeleting, setIsDeleting] = useState(false);
    const [modalData, setModalData] = useState<{ avatar: string; name: string; job: string; email: string; phone: string } | undefined>(undefined);
  ...
  const handleSave = async (event : React.FormEvent<HTMLFormElement>) => {
    ...
    const queryReq = query(staffsRef, where('email','==',emailField))
    ...
    if (!querySnapshot.empty) {
      ...
      try{
        await StaffDataService.updateStaff(docId,formattedData)
        onDataChange(true);
  ...
  const handleDeleteStaff = async () => {
    ...
    const queryEmail = query(staffsRef, where('email','==', modalData?.email))
    ...
    if (!querySnapshot.empty) {
      ...
      const docId = firstDoc.id;
      try{
        await StaffDataService.deleteStaff(docId);
        onDataChange(true);
        setModalData(undefined);
      }catch(error){
        console.log(error)
  ...
  const handleChange = (event : React.ChangeEvent<HTMLInputElement>) => {
      ...
      <tr key={item.name}>
        ...
        <td>
          <Group spacing={0} position="right">
            ...
            <ActionIcon color="red" onClick={() => handleDelete(item.name)}>
    ...
    function showEditModal(onDataChange) {
      const { avatar, name, job, email, phone } = modalData ?? {};
                                ...
                                <div>
                                  <form onSubmit={handleSave} style={{ width: '100%' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <Button type='submit' variant='gradient'>Edit Staff</Button>
                                        </div>
                                ...
                                <div>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <Group position='apart'>
                                      <Button onClick={handleDeleteStaff} variant='gradient'>Delete Staff</Button>
