...
import { Transition, Paper, Button, rem, useMantineColorScheme, createStyles, Navbar, Group, Divider, ScrollArea, Code, Text, UnstyledButton, Flex, useMantineTheme } from '@mantine/core';
...
import { CalendarEvent, Dashboard, Stack } from 'tabler-icons-react';
...
export function SideNavContainer() {
  ...
  const {loggedInUserName, loggedInUserIMG, loggedInUserEmail, isZLeader} = useContext(AppContext);
  const iconColor = colorScheme === 'dark' ? theme.white : theme.black;
  const [userNavigationDashboardPath, setUserNavigationDashboardPath] = useState('/dashboard');
  const [userNavigationDocumentPath, setUserNavigationDocumentPath] = useState('/dashboard/documents');
  const [userNavigationTransactionPath, setUserNavigationTransactionPath] = useState('/dashboard/transactions');
  const [userNavigationEventPath, setUserNavigationEventPath] = useState('/dashboard/events');
  ...
  const Logo = () => {
    ...
    return <img style={{ marginTop: '10px',height: '80px', marginLeft: '10px', width: '240px' }} src={logoSrc} alt="Barangay Sta Ana" />;
  };
  ...
  useEffect(() => {
    if (isZLeader === true){
      setUserNavigationDashboardPath('/ZoneLeader-dashboard')
      setUserNavigationDocumentPath('/ZoneLeader-dashboard/documents')
      setUserNavigationTransactionPath('/ZoneLeader-dashboard/transactions')
      setUserNavigationEventPath('/ZoneLeader-dashboard/events')
    }
  },[isZLeader])
      ...
      >
          ...
          <div>
            <Group position="apart" className={classes.header}>
                {isZLeader ? <Text>Hello Zone Leader, {formattedUserName}</Text> : <Text>Hello User, {formattedUserName}</Text>}
            </Group>
          </div>
          ...
          <div className={cx(classes.navbarsections, {
            ...
            <UnstyledButton component={Link} to={userNavigationDashboardPath}>
          ...
          <div className={cx(classes.navbarsections, {
            ...
            <UnstyledButton component={Link} to={userNavigationDocumentPath}>
          ...
          <div className={cx(classes.navbarsections, {
            ...
            <UnstyledButton component={Link} to={userNavigationTransactionPath}>
          ...
          <div className={cx(classes.navbarsections, {
            ...
            <UnstyledButton component={Link} to={userNavigationEventPath}>
          ...
          <div className={cx(classes.navbarsections, {
            ...
            <UnstyledButton className={classes.colorschemefooter}>
            <Flex className={classes.links}><ColorScheme/></Flex>
            </UnstyledButton>
          </div>} 
