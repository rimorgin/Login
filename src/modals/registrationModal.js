...
import { Stepper, Button, Group, TextInput, PasswordInput, Code, Stack, Paper, Text,Flex, Divider, createStyles, Progress, useMantineColorScheme, Select, Loader } from '@mantine/core';
...
export default function RegisterInfo () {
  ...
  const [userPurokAdd, setUserPurokAdd] = useState('');
  ...
  const handleComplete = () => {
    ...
    setUserPurokAdd(form.values.purokAddress);
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      await updateProfile(auth.currentUser, {
        displayName: userUname, photoURL: fetchURL
      }).then(() => {
        ...
        <Stepper.Step icon={<IconUserCheck size="1.1rem" color={iconColor}/>} label="Second step" description={'Personal Information'}>
          ...
          <Flex position='center' gap="md">
              ...
              <TextInput
                ...
                required={true}
                icon={<UserCircle size="0.8rem" color={iconColor}/>}
                {...form.getInputProps('purokAddress')}
              />
              <TextInput
                label="Address"
                placeholder="Address"
            ...
            <Stack maw={380} mx={'auto'}>
              <Paper shadow="xl" radius="md" p= {isLargeScreen ? 'sm' : 'xs'}>
                ...
                <Stack spacing="sm">
                  ...
                  <Flex justifyContent='space-between' gap={'10px'}><AddressBook size="1rem" color={iconColor}/> <Text color={iconColor} size={isLargeScreen ? 'sm' : 'xs'}>Purok: {userPurokAdd} </Text></Flex> 
                  <Flex justifyContent='space-between' gap={'10px'}><AddressBook size="1rem" color={iconColor}/> <Text color={iconColor} size={isLargeScreen ? 'sm' : 'xs'}>Address: {userAddress} </Text></Flex>    
